<script lang="ts">
  ...
  const yearData = $derived.by(() => {
    ...
    const months = Array.from({ length: 12 }, (_, i) => {
       ...
       const txUntilMonthEnd = transactions.filter(t => t.date <= monthEndDate);
       const totalDiffUntilMonthEnd = txUntilMonthEnd.reduce((acc, curr) => {
         return curr.type === 'income' ? acc + curr.amount : acc - curr.amount;
       }, 0);
  ...
  const monthlyContext = $derived.by(() => {
    ...
    let filteredList = [...monthTxs].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    if (filterCategory !== 'All') {
      filteredList = filteredList.filter(t => t.category === filterCategory);
    }
    ...
    const pieData = Object.keys(chartDataObj).map((key, index) => ({ 
      category: key, 
  ...
  const barChartData = $derived.by(() => {
     const data: Record<string, { month: string, income: number, expense: number, balance: number }> = {};
     transactions.forEach(t => {
       const key = t.date.substring(0, 7); 
       const dateObj = new Date(t.date);
       const month = dateObj.toLocaleDateString('fr-FR', { month: 'short', year: '2-digit' });
       if (!data[key]) data[key] = { month, income: 0, expense: 0, balance: 0 };
  ...
  const COLORS = ['#10B981', '#3B82F6', '#6366F1', '#8B5CF6', '#EC4899', '#F43F5E', '#F59E0B', '#64748B', '#0F172A'];
</script>

{#if !isLoaded}
  <div class="min-h-screen bg-slate-50 flex items-center justify-center">
    <div class="text-slate-500 animate-pulse">Chargement de vos comptes...</div>
  </div>
{:else}
  <div class="min-h-screen bg-slate-50 text-slate-800 pb-20 font-sans">
    
    <header class="bg-white border-b border-slate-200 sticky top-0 z-30 select-none drag-region shadow-sm">
      <div class="max-w-6xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <div class="flex items-center gap-3 self-start md:self-auto min-w-[200px]">
          ...
          <div>
            <h1 class="font-bold text-xl leading-tight text-slate-900">
              {viewScope === 'year' ? 'ComptaPapa' : 'Détail du Mois'}
            </h1>
            <p class="text-xs text-slate-500 font-medium">
              {viewScope === 'year' ? 'Tableau de bord annuel' : 'Gestion détaillée'}
            </p>
          </div>
        </div>
        ...
        <div class="flex items-center bg-slate-100 rounded-full p-1 shadow-inner">
           ...
           <div class="px-6 font-bold text-slate-800 w-48 text-center capitalize select-none text-lg">
             {viewScope === 'year' 
    ...
    <main class="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      {#if viewScope === 'year'}
        <div class="space-y-6 animate-in fade-in duration-300">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-slate-900 text-white p-6 rounded-2xl shadow-xl flex flex-col justify-between relative overflow-hidden group">
              <div>
                ...
                {#if isEditingBalance}
                  ...
                  />
                {:else}
                  <h2 class="text-4xl font-bold tracking-tight {globalBalance >= 0 ? 'text-emerald-400' : 'text-red-400'}">
                    {globalBalance > 0 ? '+' : ''}{globalBalance.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                  </h2>
                {/if}
              </div>
              
              <div class="mt-4 pt-4 border-t border-slate-700/50 flex justify-between items-end z-10 relative">
                  <p class="text-xs text-slate-400">
                    Variation Annuelle {currentDate.getFullYear()}: <span class="{yearData.totalVariation >= 0 ? 'text-emerald-400' : 'text-red-400'}">
                      {yearData.totalVariation > 0 ? '+' : ''}{yearData.totalVariation.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} €
                    </span>
                  </p>
              </div>
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-4 border-b border-slate-100 flex items-center gap-2">
              ...
              <h3 class="font-bold text-slate-800">Synthèse mensuelle {currentDate.getFullYear()}</h3>
            </div>
            
            <div class="overflow-x-auto">
              <table class="w-full text-left border-collapse">
                <thead>
                  <tr class="bg-slate-50 text-slate-500 text-xs uppercase font-semibold tracking-wider border-b border-slate-200">
                    <th class="px-6 py-4">Mois</th>
                    <th class="px-6 py-4 text-right text-emerald-600">Entrées</th>
                    <th class="px-6 py-4 text-right text-red-600">Sorties</th>
                    <th class="px-6 py-4 text-right text-slate-800">Résultat</th>
                    <th class="px-6 py-4 text-right text-blue-600">Solde Estimé</th>
                    <th class="px-6 py-4 text-center">Opérations</th>
                    ...
                    >
                      <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                          <span class="font-bold capitalize {isCurrent ? 'text-blue-700' : 'text-slate-700'}">
                            {month.name}
                          </span>
                          {#if isCurrent}
                            <span class="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-bold">Actuel</span>
                          {/if}
                        </div>
                      </td>
                      <td class="px-6 py-4 text-right font-medium text-emerald-600">
                        {month.income > 0 ? `+${month.income.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} €` : '-'}
                      </td>
                      <td class="px-6 py-4 text-right font-medium text-red-600">
                        {month.expense > 0 ? `-${month.expense.toLocaleString('fr-FR', { maximumFractionDigits: 0 })} €` : '-'}
                      </td>
                      <td class="px-6 py-4 text-right font-bold tabular-nums">
                         <span class="px-3 py-1 rounded-lg {month.balance >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}">
                           {month.balance > 0 ? '+' : ''}{month.balance.toLocaleString('fr-FR', { maximumFractionDigits: 2 })} €
                         </span>
                      </td>
                      <td class="px-6 py-4 text-right font-semibold">
                        <span class="px-3 py-1 rounded-lg {month.estimatedBalance >= 0 ? 'bg-blue-100 text-blue-700' : 'bg-orange-100 text-orange-700'}">
                          {month.estimatedBalance.toLocaleString('fr-FR', { maximumFractionDigits: 2 })} €
                        </span>
                      </td>
                      ...
                      <td class="px-6 py-4 text-right">
                        <ChevronRight class="inline-block text-slate-300 group-hover:text-slate-500 transition-colors" size={20} />
                      </td>
                    </tr>
                  {/each}
                </tbody>
      ...
      {#if viewScope === 'month' && monthlyContext}
        <div class="space-y-6 animate-in slide-in-from-right-4 duration-300">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-slate-100 min-h-[400px]">
            
            {#if activeTab === 'list'}
              <div class="overflow-x-auto">
                 <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 border-b border-slate-100 bg-slate-50/50">
                    <div class="bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                       <p class="text-emerald-600 text-xs font-bold uppercase">Entrées</p>
                       <p class="text-xl font-bold text-emerald-700">+{monthlyContext.stats.income.toLocaleString('fr-FR')} €</p>
                    </div>
                    <div class="bg-red-50 p-4 rounded-xl border border-red-100">
                       <p class="text-red-600 text-xs font-bold uppercase">Sorties</p>
                       <p class="text-xl font-bold text-red-700">-{monthlyContext.stats.expense.toLocaleString('fr-FR')} €</p>
                    </div>
                    <div class="p-4 rounded-xl border {monthlyContext.balance >= 0 ? 'bg-blue-50 border-blue-100' : 'bg-orange-50 border-orange-100'}">
                       <p class="text-slate-600 text-xs font-bold uppercase">Reste (Mois)</p>
                       <p class="text-xl font-bold {monthlyContext.balance >= 0 ? 'text-blue-700' : 'text-orange-700'}">
                         {monthlyContext.balance > 0 ? '+' : ''}{monthlyContext.balance.toLocaleString('fr-FR')} €
                       </p>
                    </div>
                 </div>
                    ...
                    {:else}
                      {#each monthlyContext.filteredList as tx}
                        <tr class="hover:bg-slate-50 transition-colors group">
                          ...
                          <td class="px-6 py-4 text-right font-bold tabular-nums {tx.type === 'income' ? 'text-emerald-600' : 'text-slate-800'}">
            ...
            {#if activeTab === 'chart'}
              <div class="p-8 h-[450px]">
                <h3 class="text-lg font-semibold text-slate-700 mb-6 text-center">Répartition des Dépenses</h3>
            ...
            {#if activeTab === 'evolution'}
              <div class="p-8 h-[500px]">
                <h3 class="text-lg font-semibold text-slate-700 mb-4 text-center">Historique annuel global</h3>
