<script lang="ts">
  ...
  let { onAdd, onClose, initialDate }: Props = $props();
  
  let type = $state<TransactionType>('expense');
  let amount = $state('');
  let description = $state('');
  let category = $state<Category>(CategoryValues.Food);
...
<div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
  <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-visible animate-in fade-in zoom-in duration-200">
    ...
    <form on:submit={handleSubmit} class="p-6 space-y-5">
      <div class="flex p-1 bg-slate-100 rounded-lg">
        <button
          ...
          class="flex-1 py-2 text-sm font-medium rounded-md transition-all {type === 'expense' ? 'bg-white text-red-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}"
        >
          Dépense
        </button>
        <button
          ...
          class="flex-1 py-2 text-sm font-medium rounded-md transition-all {type === 'income' ? 'bg-white text-emerald-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}"
        >
          Revenu
        </button>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="block text-sm font-medium text-slate-600 mb-1">Catégorie</label>
          <select
            bind:value={category}
      ...
      <div class="flex items-center gap-3 bg-slate-50 p-4 rounded-xl border border-slate-100 cursor-pointer hover:bg-slate-100 transition-colors" on:click={() => isRecurring = !isRecurring}>
        <div class="w-5 h-5 rounded border flex items-center justify-center transition-colors {isRecurring ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-300'}">
          {#if isRecurring}
            <Repeat size={12} />
          {/if}
        </div>
        <div class="flex-1 select-none">
          <p class="text-sm font-medium text-slate-700">Répéter tous les mois</p>
          <p class="text-xs text-slate-400">Ajoutera cette opération pour les 12 prochains mois</p>
        </div>
      </div>
      ...
      >
        <Plus size={20} />
        {isRecurring ? 'Ajouter pour toute l\'année' : "Ajouter l'opération"}
      </button>
    </form>
  </div>
</div>
